...
def clean_dataframe(dataframe):
    ...
    filtered.rename(columns={'DATOS1': 'HISTORIAL'}, inplace=True) ## Se renombran las columnas de manera más descriptiva
    filtered = filtered.apply(clean_historial) ## Se limpia la columna de historial para extraer solo los cierres de OT
...
def clean_historial(dataframe):
    historial = dataframe["HISTORIAL"]
    delimiter = "-"*40

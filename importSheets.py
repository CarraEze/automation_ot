...
## Funcion para obtener el path global, ya que al generar el exe, no se pueden direccionar archivos de manera comun
def get_base_path():
...
## Funcion para obtener los parametros de configuracion
def get_config():
    with open(base_path + "\\" + "config.json", "r") as file:
        (worksheet_id, file_path) = json.load(file).values()
    if not worksheet_id or not file_path:
        raise ValueError("Some info is missing in the config file")
    worksheet_id = worksheet_id.split("/d/")[1].split("/")[0] if "https://docs.google.com/spreadsheets/d/" in worksheet_id else worksheet_id
    return (worksheet_id, file_path)

(worksheet_id, file_path) = get_config()
...
## Clase para manejar los eventos del sistema de archivos, específicamente la creación o modificación de archivos en la carpeta monitoreada.
class FolderHandler(FileSystemEventHandler):
    ...
    def process(self, event):
        ...
        file_event_path = event.src_path
        now = time.time()

        # Check last execution time
        if file_event_path in last_execution:
            elapsed = now - last_execution[file_event_path]
            if elapsed < COOLDOWN:
                return

        # Update timestamp
        last_execution[file_event_path] = now

        print(f"Processing: {file_event_path}")
